...
export const SearchDropdown: React.FC<SearchDropdownProps> = ({
  ...
  const typeTagStyle = (type: string): React.CSSProperties => ({
    ...
    backgroundColor:
      type === 'movie' ? watchaTokens.colors.primary : type === 'actor' ? '#00c896' : '#ff6b6b',
    color: 'white',
    lineHeight: 1,
  });

  // 검색어 강조 효과 제거 - 단순히 텍스트만 반환
  const highlightQuery = (text: string, _query: string) => {
    return text;
  };
      ...
      {items.map((item, index) => (
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <span style={typeTagStyle(item.key_type)}>
              {item.key_type === 'movie' ? '영화' : item.key_type === 'actor' ? '배우' : '감독'}
            </span>
            <span style={{ color: watchaTokens.colors.text.primary }}>
              {highlightQuery(item.key_display, query)}
            </span>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
